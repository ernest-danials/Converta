//
//  EditHistoricalViewBaseCurrencyView.swift
//  Converta
//
...
struct EditHistoricalViewBaseCurrencyView: View {
    ...
    var body: some View {
        ...
        .onChange(of: self.numberValue) { newValue in
            withAnimation {
                if newValue.isEmpty {
                    subViewModel.updateBaseAmountFromString(string: "1.00", viewModel: viewModel)
                } else {
                    subViewModel.updateBaseAmountFromString(string: newValue, viewModel: viewModel)
                }
            }
        }
    ...
    func setUp() {
        withAnimation {
            ...
            if subViewModel.baseAmount == 1.00 {
                ...
                self.numberValue = String(format: "%.\(currentCurrencyDecimalDigit ?? 2)f", subViewModel.baseAmount)
            }
        ...
        VStack {
            ...
            Button {
                ...
                HStack {
                    ...
                    Text(subViewModel.baseCurrency.rawValue)
                        .customFont(size: 30, weight: .semibold)
                ...
                VStack(spacing: 10) {
                    ...
                    if searchText.isEmpty && !viewModel.favoriteCurrencies.isEmpty {
                        ...
                        ForEach(viewModel.favoriteCurrencies, id: \.self) { rawValue in
                            ...
                            Button {
                                withAnimation {
                                    subViewModel.updateBaseCurrency(to: code ?? .USDollar, date: date)
                                ...
                                HStack {
                                    ...
                                    if subViewModel.baseCurrency == code {
                    ...
                    ForEach(CurrencyCode.allCases.filter { $0.rawValue.hasPrefix(searchText.uppercased()) }, id: \.self) { code in
                        ...
                        Button {
                            withAnimation {
                                subViewModel.updateBaseCurrency(to: code, date: date)
                                numberValue = ""
                                toggleCurrentEditStatus()
                            }
                        } label: {
...
struct EditHistoricalViewBaseCurrencyView_Previews: PreviewProvider {
    static var previews: some View {
        EditHistoricalViewBaseCurrencyView(subViewModel: .init(), date: .now).environmentObject(ViewModel())
    }
}
