//
//  EditCryptoCurrencyViewBaseCurrencyView.swift
//  Converta
//
...
struct EditCryptoCurrencyViewBaseCurrencyView: View {
                ...
                VStack(spacing: 10) {
                    ...
                    if searchText.isEmpty && !viewModel.favoriteCurrencies.isEmpty {
                        ...
                        ForEach(viewModel.favoriteCurrencies, id: \.self) { rawValue in
                            ...
                            Button {
                                withAnimation {
                                    subViewModel.updateBaseCurrency(to: code ?? .USDollar)
                    ...
                    ForEach(CurrencyCode.allCases.filter { $0.rawValue.hasPrefix(searchText.uppercased()) }, id: \.self) { code in
                        ...
                        Button {
                            withAnimation {
                                subViewModel.updateBaseCurrency(to: code)
                                numberValue = ""
                                toggleCurrentEditStatus()
                            }
                        } label: {
...
struct EditCryptoCurrencyViewBaseCurrencyView_Previews: PreviewProvider {
    static var previews: some View {
        EditCryptoCurrencyViewBaseCurrencyView(subViewModel: .init())
    }
}
